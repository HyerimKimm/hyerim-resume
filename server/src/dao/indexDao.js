...
exports.selectProjectsByResumeId = async function (connection, resumeId) {
  const selectProjectByResumeIdQuery = `SELECT projectId, projectName, startDate, endDate, deployLink, repositoryLink, links, imgUrl, staff, description, responsibilities, results, isServiceEnd, orderNum FROM Projects WHERE resumeId = ? ORDER BY orderNum ASC`;
